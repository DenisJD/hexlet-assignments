...
public class AppTest {
    ...
    // BEGIN
    @Test
    void testAdvFilter() throws Exception {
        MockHttpServletResponse response = mockMvc
                .perform(get("/users/adv?gender=Male&email=t&lastName=like"))
                .andReturn()
                .getResponse();
        ...
        User actualUser = actualUsers.get(0);
        assertThat(actualUsers.size()).isEqualTo(1);
        assertThat(actualUser.getFirstName()).isEqualTo("Waylon");
        assertThat(actualUser.getLastName()).isEqualTo("Stanlike");
    }
    // END
}
